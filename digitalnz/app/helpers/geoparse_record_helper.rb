...
=end
          ...
          puts keepers.to_yaml
          
          
          for term in keepers
              puts "TERM:#{term} added to "
              #Was p = term.cached_geo_search_term.search_term
              for cgst in term.cached_geo_search_terms
                  p = cgst.search_term
          ...
          for cached_place in keepers
             ...
             place_string = "\t#{cached_place.cached_geo_search_terms.map{|s|s.search_term}}\t#{cached_place.country} - #{cached_place.accuracy.name} - '#{cached_place.address} - #{cached_place.latitude}, #{cached_place.longitude} - ' D:#{delta_from_centroid}"
